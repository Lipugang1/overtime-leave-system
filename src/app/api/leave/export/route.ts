...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase
      .from('leave_requests')
      .select('*, employee:employees!leave_requests_employee_id_fkey(name, username, position, department, module), level1_approver:employees!leave_requests_level1_approver_id_fkey(name), level2_approver:employees!leave_requests_level2_approver_id_fkey(name)')
    ...
    // 生成CSV
    const headers = ['工号', '姓名', '岗位', '部门', '模块', '开始时间', '结束时间', '工时(小时)', '原因', '状态', '一级审批人', '二级审批人'];
    ...
    return new NextResponse(csvBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename=leave_export_${new Date().toISOString().split('T')[0]}.csv`,
      },
    });
  } catch (error) {
